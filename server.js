...
// Runs the emoji/word analysis on whatever rows you hand it —
// works the same whether it's all messages or just one month's worth.
function analyzeMessages(rows) {
  ...
  for (const row of rows) {
    ...
    if (cryMatches) cryCount += cryMatches.length;
...
// Turns a [emoji, count] or [word, count] array into HTML rows
function buildRows(entries, formatLabel) {
  return entries
    .map(([label, count]) => `
      <div class="row">
        <span class="row-label">${formatLabel ? formatLabel(label) : label}</span>
...
app.get('/', (req, res) => {
  ...
  // THIS MONTH's stats — ?month=YYYY-MM in the URL, defaults to current month
  const now = new Date();
